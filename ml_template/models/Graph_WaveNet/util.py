...
class DataLoader(object):
    ...
    def shuffle(self, seed=None):
        ...
        xs, ys, cs = self.xs[permutation], self.ys[permutation], self.cs[permutation]
        self.xs = xs
        self.ys = ys
        self.cs = cs
...
def load_dataset(dataset_dir, batch_size, valid_batch_size= None, test_batch_size=None):
    ...
    data['train_loader'] = DataLoader(data['x_train'], data['y_train'], batch_size, cs=data['c_train'])
    data['val_loader'] = DataLoader(data['x_val'], data['y_val'], valid_batch_size, cs=data['c_val'])
    data['test_loader'] = DataLoader(data['x_test'], data['y_test'], test_batch_size, cs=data['c_test'])
    data['scaler'] = scaler
...
def masked_mape(preds, labels, null_val=np.nan, params=None):
    if np.isnan(null_val):
        mask = ~torch.isnan(labels)
    else:
        mask = (labels != null_val)
...
def metric(pred, real, params=None):
    mae = masked_mae(pred, real, 0.0, params=params).item()
    mape = masked_mape(pred, real, 0.0, params=params).item()
    rmse = masked_rmse(pred, real, 0.0, params=params).item()

...
from models.GTS.model.pytorch.cell import DCGRUCell
import numpy as np
...
class EncoderModel(nn.Module, Seq2SeqAttrs):
    def __init__(self, config, **model_kwargs):
        ...
        self.seq_len = config.seq_length  # for the encoder
...
class DecoderModel(nn.Module, Seq2SeqAttrs):
    def __init__(self, config, **model_kwargs):
        ...
        self.horizon = config.output_horizon  # for the decoder
...
class GTSModel(nn.Module, Seq2SeqAttrs):
    def __init__(self, config, logger, **model_kwargs):
        super().__init__()
        Seq2SeqAttrs.__init__(self, **model_kwargs)
        self.encoder_model = EncoderModel(config, **model_kwargs)
        self.decoder_model = DecoderModel(config, **model_kwargs)
        self.cl_decay_steps = int(model_kwargs.get('cl_decay_steps', 1000))
    ...
    def decoder(self, encoder_hidden_state, adj, labels=None, batches_seen=None):
        ...
        embs = decoder_hidden_state[0]
        embs = torch.reshape(embs, (batch_size, self.num_nodes, self.rnn_units))
        #  embs should be of shape (batch_size, emb_size, num_nodes, 1) to reuse the code of concept loss computation
        embs = torch.unsqueeze(embs, dim=-1)
    ...
    def forward(self, label, inputs, node_feas, temp, gumbel_soft, labels=None, batches_seen=None):
        ...
        outputs, embs = self.decoder(encoder_hidden_state, adj, labels, batches_seen=batches_seen)
        ...
        return outputs, x.softmax(-1)[:, 0].clone().reshape(self.num_nodes, -1), embs
